...
* You're here because your program of study requires you to learn stats procedures such as t and chi-square tests, regression etc.

* Some of you may plan to use stats in your future work. Great!

* But others may plan to sell the text and forget everything after the final exam. That's fine, too.

* What is most important to me is that you get some idea of how to think statistically from this course. 
    + Specific techniques are less important than the general way of thinking. 
    + If your program wants me to show you t tests, or chi-squared tests, etc. fine.

* I'll try to bring in the bigger ideas: quantifying uncertainty and avoiding bias to make inference from data. 
    + The syllabus is tilted towards quantifying uncertainty rather than bias, but I'll try to work in both throughout the course.
...
by repeatedly drawing samples of data from the population,
...
* Statistical inference: Using data to drawing conclusions about  _parameters_.
\medskip
...
* Estimates of a parameter without some indication of precision are useless.
* Confidence intervals (CIs), sometimes called interval estimates,
...
level-$C$ CI for
$\mu$ is $\bar{x} \pm z^* \times \sigma/\sqrt{n}$
...
* E.G. 95\% CI: If $\bar{X}$ is normally distributed, it will
be within $z^*=1.96$ of its SDs (i.e. within $1.96 \times \sigma/\sqrt{n}$) of
$\mu$ 95\% of the time.
    + So $\bar{X} \pm 1.96 \times \sigma/\sqrt{n}$ will cover $\mu$ 95\% of the
time.
* The margin of error gets smaller as the confidence level $C$ gets smaller, the population standard deviation $\sigma$ gets smaller and/or the sample size
$n$ gets bigger

## CI for a Mean -- Unknown SD

* If the SD $\sigma$ is unknown, we estimate it by the sample SD, $s$. 
* A level-$C$ CI for
$\mu$ is then $$\bar{x} \pm t^* \times s/\sqrt{n},$$
where $t^*$ is the upper $(1-C)/2$ critical value of the appropriate $t$ distribution
* $t$ distributions are similar in shape to a standard-normal distribution, but with
slightly heavier tails.
* $t$ distributions are indexed by a parameter called the degrees of 
freedom (df). 
...
critical value for the $t$-distribution with $n-1=9$ df.
    + Statistical tables, such as Table A.4 of the text, or
    statistical software tell us
    that the critical value is $t^*= 2.262$.
...
* Some key points to watch for:
    + Hypotheses are in terms of population parameters of interest
    + The alternative hypothesis can be one-sided or two-sided 
    + We use a test statistic with a known sampling 
...
* Use $\bar{X}$ as a proxy for $\mu$
* Observed values $\bar{x}$ that are far from $\mu_0$ are taken as evidence 
in favour of $H_a$.
...
* From our discussion of what "extreme" means 
under various alternative hypotheses, we
can argue that:
    + For $H_a: \mu \not= \mu_0$,
$p=2P(T\geq|t|)$, where
...
which computer software reports to be `1.36833e-07`.
    ...
    $0.000000136833$ 
    + decimal, six zeros, then the number

## Hypothesis Tests

* Small $p$-values are evidence against the null hypothesis, in favour
of the alternative hypothesis.
* We may set a level $\alpha$ in advance that marks the point at which
evidence against the null hypothesis is considered strong enough to "reject" it.
    + If we don't reject the null hypothesis we retain it.
* If the $p$-value is less than $\alpha$, we say that we "reject $H_0$ at level $\alpha$".
...
practical significance.
    + For example, in a large clinical trial, a drug may lower cholesterol ever-so-slightly,
...
parameters from data that are subject to random variation.
