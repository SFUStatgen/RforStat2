...
* *Example:* 
Estrogen $+$ progestin (EP) or not ($\overline{\rm EP}$)
...
* E.G. Random experiment of 2 tosses of a fair coin
has sample space ${\cal S}=\{HH, HT, TH, TT \}$,
with all 4 outcomes having chance $1/4$.

* Knowing $H$ occurs on the 1st toss provides no info about whether $H$ will occur in the 2nd toss. 

    +  Let $A = \{ HH, HT \}$ be the event of an $H$ on 
    the 1st toss and $B=\{ HH, TH \}$ be the event of an $H$ on the 2nd toss. 
...
* In the coin-toss experiment, $A \cap B = \{ HH \}$.
    + $P(A)=P(\{HH, HT\})=P(HH)+P(HT)=1/4+1/4=1/2$,  $P(B)=P(\{HH, TH\})=P(HH)+P(TH)=1/4+1/4=1/2$.
...
* When $A$ and $B$ are independent, 
    ...
    $$P(A \cap B) = P(A) \times P(B).$$ 
* In general, though, knowing
$B$ has happened may modify the probability of $A$; 
...
positive and negative predictive values.
    +  Samples with few diseased persons yield unreliable estimates of
        +  $P(D^+\mid T^+)$, the positive predictive value, or
...
* Importantly, the odds of exposure in either cases or controls _can_
