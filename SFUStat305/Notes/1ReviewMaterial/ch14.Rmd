...
subtitle: 'Chapter 14 : Inference for Difference between Proportions'
...
## Inference for Difference between Proportions (Chapter 14)
...
the approximate sampling distribution of a pivotal quantity, $Z$,
that depends on $\hat{p}_1 - \hat{p}_2$.
...
* The normal approximation to the distribution of $Z$ is reliable when the sample sizes $n_1$ and $n_2$ are "large"
relative to the $p_j$'s.
<!--
...
-->
* The parameters $p_1$ and $p_2$ are not known so estimates
$$\hat{p}_1=(\mbox{number of successes in sample 1})/n_1 $$ 
$$\hat{p}_2= (\mbox{number of successes in sample 2})/n_2$$
are used instead to get the following rule:
