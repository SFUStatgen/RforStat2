...
* Want to make inference about the difference between 
...
* The averages $\bar{x}_1$ and $\bar{x}_2$ estimate the means
...
* Compare variables such as age and birth weight in moms with vs. without toxemia 
...
* Explore differences graphically with boxplots (below) and 
with histograms (later, two slides from now) by toxemia 
...
distribution of $T$.
* Note: We will omit the following topics in text:
...
the **non-toxemia** (`toxCateg` 0) and **toxemia** (`toxCateg` 1)
groups."
...
or 0.000094.
    ...
    random variable, $T$, having a t distribution 
...
gestational age are **associated**.
    ...
    in the two toxemia groups (different means).
