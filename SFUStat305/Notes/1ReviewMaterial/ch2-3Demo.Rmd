...
* Most of the data analyses we do in R is with functions.
* You are familiar with functions such as `log` on your calculator 
that take a number as input, and output another number.
...
* R function output:
    + The output, or what the function "returns", is an R object that summarizes
    the computations or other work done by the function.
    
## Softward Notes: R functions, cont.

* `log(x)` takes a number `x` as input and returns its natural log; e.g.,
...
* `ls()` lists the R objects in your workspace -- no arguments needed.
